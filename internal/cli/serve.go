...
func newServeCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "serve",
		Short: "Run the gateway HTTP server (default when no subcommand is given)",
...
// runServe is the gateway's server mode: everything cmd/gateway/main.go
// used to do directly. It calls os.Exit itself on fatal startup errors —
// this is the process's main job when running as a server, so that
// matches the rest of this repo's service binaries.
func runServe() {
	cfg, err := config.Load()
	if err != nil {
